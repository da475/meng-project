...
class DATASET_MGR:
    def __init__(self, filename):
        ...
        data_size = ord(byte[0]) << 24 | ord(byte[1]) << 16 | ord(byte[2]) << 8 | ord(byte[3])

        # read image height
        byte = self.data_file.read(4)    # bytes for no of images
        image_rows = ord(byte[0]) << 24 | ord(byte[1]) << 16 | ord(byte[2]) << 8 | ord(byte[3])
        
        # read image width
        byte = self.data_file.read(4)    # bytes for no of images
        image_cols = ord(byte[0]) << 24 | ord(byte[1]) << 16 | ord(byte[2]) << 8 | ord(byte[3])
    ...
    def load_data(self):
        ...
        for image_index in xrange(max_image_index):
            for pixel_index in xrange(max_pixel_index):
                byte = self.data_file.read(1)  # read pixel of 1 byte
                self.data_array[image_index, pixel_index] = float(ord(byte))
